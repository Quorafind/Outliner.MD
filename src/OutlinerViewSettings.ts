...
export class OutlinerViewSettingTab extends PluginSettingTab {
	...
	initGlobalSettings(containerEl: HTMLElement, settings: OutlinerViewSettings) {
		const editorSettingDesc = document.createDocumentFragment();

		editorSettingDesc.createEl('div', {text: 'You can disable some internal editor plugins from Outliner.md.'});
	...
	initBacklinkSettings(containerEl: HTMLElement, settings: OutlinerViewSettings) {
		...
		backlinkSettingDesc.createEl('div', {text: 'Backlinks are the links that point to the current note. You can make backlinks editable and enable live preview for backlinks.'}).createEl('span', {text: ' Note: '}).createEl('a', {
		...
		backlinkSettingName.createEl('span', {text: ' Backlink'});

		new Setting(containerEl).setHeading().setName(backlinkSettingName).setDesc(backlinkSettingDesc);
	...
	initBlockEmbedSettings(containerEl: HTMLElement, settings: OutlinerViewSettings) {
		...
		blockEmbedSettingDesc.createEl('div', {text: 'Block embeds are the blocks that are embedded in the current note. You can make block embeds editable.'}).createEl('span', {text: ' Note: '}).createEl('a', {
			text: 'Block embeds',
		...
		new Setting(containerEl).setHeading().setName(blockEmbedSettingName).setDesc(blockEmbedSettingDesc);
	...
	initPageBookSettings(containerEl: HTMLElement, settings: OutlinerViewSettings) {
		...
		descriptionFragment.createEl('div', {
		...
		}).createEl('span', {text: ' Note: '}).createEl('a', {
			text: 'Note as book',
			attr: {href: 'https://docs.outliner.md/pages/20240517162521'}
		});

		new Setting(containerEl).setHeading().setName(nameFragment).setDesc(descriptionFragment);


		new Setting(containerEl)
			.setName('Turn the note into a notebook')
			.addToggle(toggle => toggle.setValue(settings.noteAsNotebook).onChange(async (value) => {
		...
		if (settings.noteAsNotebook) {
			...
			sectionFragment.createEl('p', {text: 'The mark to split the note into sections. Currently, it is not configurable.'});
			...
			sectionFragment.createEl('span', {text: ' Note: '}).createEl('a', {
				text: 'Lineage',
				attr: {href: 'https://github.com/ycnmhd/obsidian-lineage'}
			});
			...
			new Setting(containerEl)
				...
				.addToggle(toggle => toggle.setValue(settings.alwaysShowSectionHeader).onChange(async (value) => {
			...
			new Setting(containerEl)
				...
				.addToggle(toggle => toggle.setValue(settings.showFullBtnAtLeftSide).onChange(async (value) => {
					settings.showFullBtnAtLeftSide = value;
					this.applySettingsUpdate();

					document.body.toggleClass('omd-change-section-btns-order', value);
				}));

			new Setting(containerEl)
				.setName('Hide empty section header automatically')
				.addToggle(toggle => toggle.setValue(settings.autoHideEmptySectionHeader).onChange(async (value) => {
	...
	initTaskGroupSettings(containerEl: HTMLElement, settings: OutlinerViewSettings) {
		...
		taskGroupSettingDesc.createEl('div', {text: 'Task group is a group of tasks that you can fold and unfold. You can set a custom query string for task group.'}).createEl('span', {text: ' Note: '}).createEl('a', {
			text: 'Task group',
		...
		new Setting(containerEl).setHeading().setName(taskGroupSettingName).setDesc(taskGroupSettingDesc);
		...
		new Setting(containerEl)
			...
			.setDesc('Custom query string for task group, you can set a tag or word for task querying. After changing this setting, you need to reload the page.')
			.addText(text => {
				text.setPlaceholder('#now').setValue(settings.taskGroupQuery).onChange(async (value) => {
	...
	styleSettings(containerEl: HTMLElement, settings: OutlinerViewSettings) {
		...
		styleSettingDesc.createEl('div', {text: 'You can change the style of Outliner.md.'});
		...
		new Setting(containerEl).setHeading().setName(styleSettingName).setDesc(styleSettingDesc);

		const paperLayoutSettingDesc = document.createDocumentFragment();
		paperLayoutSettingDesc.createEl('div', {text: 'Paper layout is a layout that looks like a paper. You can enable or disable it.'}).createEl('span', {text: ' Note: '}).createEl('a', {
			text: 'Paper layout',
		...
		moreStyleSettings.createEl('div', {text: 'More style settings are in the Obsidian style settings plugin.'}).createEl('span', {text: ' Note: '}).createEl('a', {
			text: 'Obsidian style settings',
			attr: {href: 'https://github.com/mgmeyers/obsidian-style-settings'}
		});
		...
		new Setting(containerEl)
			...
			.addButton(button => button.setButtonText('Open').onClick(() => {
				const existStyleSettingsPlugin = this.app.plugins.getPlugin('obsidian-style-settings');
