...
export const zoomInRangesEffect = StateEffect.define<{
	ranges: Array<{ from: number; to: number }>;
}>();

export const hideRangesEffect = StateEffect.define<{
	ranges: Array<{ from: number; to: number }>;
}>();
...
export const zoomWithHideIndentEffect = StateEffect.define<{
	range: { from: number; to: number };
	indent: string;
}>();

export const hideFrontMatterEffect = StateEffect.define<{
	range: { from: number; to: number };
}>();
...
export class EditWidget extends WidgetType {
	...
	eq(widget: WidgetType): boolean {
		return (
			(widget as EditWidget).from === this.from &&
			(widget as EditWidget).to === this.to
		);
	}
	...
	toDOM(): HTMLElement {
		...
		new ExtraButtonComponent(el).setIcon("pencil").onClick(() => {
			...
			if (embeddedContainer && "cmView" in embeddedContainer) {
				const widget = (embeddedContainer as any).cmView.widget;
				const cm = (widget.editor as any).cm;
				cm.focus();
				cm.dispatch({
					selection: { head: widget.start, anchor: widget.end },
					scrollIntoView: true,
				});
			}
		});
					...
					value = value.update({
						add: [
							Decoration.widget({
								...
								side: 1,
							}).range(e.value.to + 1, e.value.to + 1),
						],
					});
				}
						...
						add: [
							e.value.type === "part"
								? Decoration.replace({
										block: true,
										inclusiveEnd: false,
								}).range(0, e.value.from - 1) 
								: zoomMarkHidden.range(0, e.value.from - 1),
						],
					});
				}
					...
					value = value.update({
						add: [
							e.value.type === "part"
								...
								}).range(e.value.to + 1, tr.newDoc.length)
								: zoomMarkHidden.range(
										e.value.to + 1,
										tr.newDoc.length
								),
						],
			...
			if (e.is(zoomInRangesEffect)) {
				...
				hiddenRanges.forEach((range) => {
					value = value.update({
						add: [
							zoomMarkHidden.range(
								range.from === 0 ? range.from : range.from + 1,
